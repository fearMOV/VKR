...
def main():
    ...
    """
    global k, config  # Добавляем переменные пространство имён функции из глобалного пространства имён

    # Подключаемся к PostgreSQL и делаем запросы
    with psycopg2.connect(**config["postgresql"]) as conn:  # Подключаемся к PostgreSQL
        logger.info("Подключение к БД PostgreSQL успешно")
...
Неактуальные пропущенные вызовы: {}".format(queued_calls_without_irrelevant_missed_calls))  # Логируем словарь
    ...
    for key, value in sl_projects.items():  # Перебираем словарь с помощью items() получаем отдельно ключ и значение
        data_json["projects"] += [{key: [{"el": value, "ewt": ewt[key]}]}]  # Создаём словарь
...
logging.basicConfig(
    filename="sample.log",  # Название файла лога
...
# handler_root = logging.FileHandler(filename='sample.log', mode="a")  # Название файла
# handler_root.setFormatter(formatter)  # Задаём формат выводимых сообщений
# handler_root.setLevel(logging.DEBUG)  # Задаём уровень выводимых сообщений
# logger.addHandler(handler_root)  # Добавляем объект handler к logger
...
try:  # Ловим ошибки
    ...
    with open(parser.parse_args().config_file) as inf:  # Открывем конфигурационный файл.
